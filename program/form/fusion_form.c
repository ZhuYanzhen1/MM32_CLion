...
void gui_show_fusion(void *parameter) {
    (void) parameter;
    gui_label_settext(&north_label, " north:%.2f", small_packets.north);
    ...
    gui_label_settext(&north_distance_label, " nd:%.3f", neu.north_distance);
    gui_label_settext(&east_distance_label, " ed:%.3f", neu.east_distance);
