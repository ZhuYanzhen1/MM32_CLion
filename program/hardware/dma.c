...
void uart2_dma_set_transmit_buffer(const unsigned int *data_address, unsigned short data_length) {
    DMA1_Channel6->CNDTR = (volatile unsigned int) data_length;
    DMA1_Channel6->CMAR = (volatile unsigned int) data_address;
    MODIFY_REG(DMA1_Channel6->CCR, DMA_CCR_EN, 1 << DMA_CCR_EN_Pos);
}
...
void uart2_dma_receive_config(const unsigned int *data_address, unsigned short data_length) {
    ...
    DMA_StructInit(&DMA_InitStruct);
    DMA_InitStruct.DMA_PeripheralBaseAddr = (unsigned int) &UART2->RDR;
    DMA_InitStruct.DMA_MemoryBaseAddr = (unsigned int) data_address;
    DMA_InitStruct.DMA_DIR = DMA_DIR_PeripheralSRC;
    DMA_InitStruct.DMA_BufferSize = data_length;
    DMA_InitStruct.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
    ...
    UART_DMACmd(UART2, UART_GCR_DMA, ENABLE);
...
void uart2_dma_nvic_config() {
    ...
    NVIC_InitStruct.NVIC_IRQChannelPreemptionPriority = UART2_DMA_PRIORITY;
    NVIC_InitStruct.NVIC_IRQChannelSubPriority = 0;
    NVIC_InitStruct.NVIC_IRQChannelCmd = ENABLE;
    exNVIC_Init(&NVIC_InitStruct);
}
